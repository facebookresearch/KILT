...
class WizardDataset(Dataset):
    def __init__(self, name, input_file, output_file, log_file):
        super().__init__(name)
        self.input_file = "/private/home/edinan/ParlAI/data/wizard_of_wikipedia/train.json"
    ...
    def process_chunk(self, chunk, ks, chunk_id=-1):
        ...
        for idx, datapoint in enumerate(chunk):
            ...
            episode_length = 0
            all_dialogue = datapoint['dialog']
            ongoing_dialogue = []
            for dialog in all_dialogue:
                ongoing_dialogue.append(dialog['text'])
                if 'checked_sentence' in dialog:
                    episode_length += 1
                    if dialog['checked_sentence'] != {}:
                        checked_sentence = dialog['checked_sentence']
                        # import pdb; pdb.set_trace();
                        topic = " ".join(list(checked_sentence.keys())[0].split("_")[1:-1])
                        print(topic)

                        page = ks.get_pages_by_title(topic)
                        if not page:
                            missing_pages += 1
                        else:
                            page = page[0]
                            kilt_record = {
                                ...
                                "input": [turn for turn in ongoing_dialogue],
                            }
                            enumerate_id += 1
                            ...
                            chosen_answer = list(checked_sentence.values())[0]
                            answer_span = chosen_answer
